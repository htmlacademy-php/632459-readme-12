<main class="page__main page__main--messages">
    ...
    <section class="messages tabs"
             style="<?= empty($dialogs_users) ? 'display: block;' : '' ?>">
        <h2 class="visually-hidden">Сообщения</h2>
        <div class="messages__contacts"
             style="<?= empty($dialogs_users) ? 'width: auto; margin-left: 0'
                 : '' ?>">
            <ul class="messages__contacts-list tabs__list">
                <?php
                if (empty($dialogs_users)): ?>
                    <p style="width: auto; font-size: 24px; margin: 100px 0 0 25%;">
                        У вас пока нет активных диалогов</p>
                <?php
                endif; ?>
                <?php
                if (!empty($dialogs_users)): ?>
                <?php
                foreach ($dialogs_users as $user): ?>
                    <li class="messages__contacts-item">
                        <a class="messages__contacts-tab tabs__item tabs__item--active <?= $user['id']
                        === $first_user ? 'messages__contacts-tab--active'
                            : '' ?> "
                           href="/messages.php?user=<?= $user['id'] ?? '' ?>">
                            <div class="messages__avatar-wrapper">
                                ...
                                <?php
                                if (isset($unread_msg[$user['sender']]) && $unread_msg[$user['sender']] > 0): ?>
                                    <i class="messages__indicator"><?= $unread_msg[$user['sender']]
                                        ?? '' ?></i>
                                <?php
                                endif; ?>
                            </div>
            ...
            <div class="comments">
                ...
                <textarea class="comments__textarea form__textarea form__input"
                        ...
                        <div class="form__error-text">
                            ...
                            <p class="form__error-desc"><?= $errors['message']
                                ?? '' ?></p>
                        </div>
                    </div>
